...
- `forcats::fct_recode()` changes the column labels to "Departure" and "Return" for clarity. `forcats::fct_rev()` reverses the order in which the factors appear on the graph. Character/text variables can be converted to factors by `factor()` or `forcats::as_factor()` (which use different rules to determine the factor order).
...
df <- read_csv("https://raw.githubusercontent.com/CMAP-REPOS/cmap_abm_report/master/data/csv37_tours_tod.csv") %>%
  filter(subgroup %in% c("Model-Depart", "Model-Arrive")) %>% 
  filter(cat %in% c("work", "school", "discretionary")) %>% 
  mutate(
    purpose = factor(stringr::str_to_title(cat)),  # Title case
    dep_arr = fct_rev(fct_recode(factor(subgroup), 
...
p <- ggplot(data = df, mapping = aes(x = hours, y = value)) +
  ...
  geom_col(  # Or geom_histogram() if data is not already grouped
    mapping = aes(fill = purpose)#dep_arr)  # Color bars by tour purpose
  ) +
  
  # Facet horizontally by tour purpose
  facet_grid(rows = vars(dep_arr))+#purpose)) +
...
finalize_plot(
  p,
  title = "Modeled departure and return times for work, school and discretionary
    tours in CMAP's Activity-Based Model",
