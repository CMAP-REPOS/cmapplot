...
Line graphs are useful for showing change over time, and are well-suited for combination with `geom_text_lastonly()` and `geom_recessions()` custom geoms. This particular plot also demonstrates the utility of a few other `ggplot2`-adjacent functions from the tidyverse:
...
p <- ggplot(data = df, mapping = aes(
  ...
  scale_y_continuous(
    labels = scales::label_percent(accuracy = 1),      # Format y-axis numbers with commas
    expand = expansion(add = 0)) +                     # No padding beyond specified limits
  
  # establish an attractive X scale
  scale_x_continuous(
    breaks = seq(min(df$year), max(df$year), by = 2),  # Set breaks to every other year
    expand = expansion(add = c(0.5, 1.25))) +          # Set custom padding for L and R side of axis 
...
# Create final plot
finalize_plot(
  p, mode = "png", filename = "./cookbook_line.png",
  
  # modify default sizes a touch
  height = 4.5,
  title_width = 2.6,
  
  # use some <br> tags to get the title and caption to match perfectly
  title = "Cumulative growth<br>in real gross regional product, 2001-19",
  caption = "Note: Indext year 2001. <br><br> Source: Chicago Metropolitan
    Agency for Planning analysis of U.S. Bureau <br>of Economic Analysis and 
    Economic Modeling Specialists International <br>data (Emsi 2020.4).",
  
  # change some plotting constants
  overrides = list(margin_plot_r = 0,    # eliminate distance btwn plot and right side of image
                   margin_legend_b = 25) # larger than usual gap between legend and plot
)

```

<img src="cookbook_line.png" width="100%" style="display: block; margin: auto;"/>

#### Original graphic

<img src="https://www.dropbox.com/s/xol70o61citlpjj/Cumulative_Growth-01.png?raw=1" width="100%" style="display: block; margin: auto;"/>

[Source](https://www.dropbox.com/s/xol70o61citlpjj/Cumulative_Growth-01.png?dl=0)


## Area plot

Area plots are useful for showing changes in composition over time. (See also: stacked bar charts.)
...
# Create final plot
finalize_plot(
  p, mode = "png", filename = "./cookbook_area.png",
...
<img src="cookbook_area.png" width="100%" style="display: block; margin: auto;"/>
...
# Create final plot
finalize_plot(
  p, mode="png", filename="cookbook_scatter.png",
...
<img src="cookbook_scatter.png" width="100%" style="display: block; margin: auto;"/>
...
# Create final plot
finalize_plot(
  p, mode = "png", filename = "./cookbook_highlightbar.png",
...
<img src="cookbook_highlightbar.png" width="100%" style="display: block; margin: auto;"/>
...
p <- ggplot(data = df,
            mapping = aes(
              ...
              y = RESPONSE, fill = YEAR, label = LABEL
            )) +
  ...
  # Label bars
  geom_text(
    mapping = aes(y = RESPONSE + .025),  # Shift upward to eliminate overlap
...
# Create final plot
finalize_plot(
  p, mode = "png", filename = "./cookbook_clusterbar.png",
...
<img src="cookbook_clusterbar.png" width="100%" style="display: block; margin: auto;"/>
...
# Create final plot
finalize_plot(
  p, mode="png", filename="cookbook_stackbar.png",
...
<img src="cookbook_stackbar.png" width="100%" style="display: block; margin: auto;"/>
...
# Create final plot
finalize_plot(
  p, mode="png", filename="cookbook_donut.png",
...
<img src="cookbook_donut.png" width="100%" style="display: block; margin: auto;"/>
...
# Create final plot
finalize_plot(
  p, mode="png", filename="cookbook_racepalette.png",
...
<img src="cookbook_racepalette.png" width="100%" style="display: block; margin: auto;"/>
