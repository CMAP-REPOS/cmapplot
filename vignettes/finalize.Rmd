...
In addition to setting the plot's `height` and `width`, `finalize_plot()` has a number of top-level arguments that impact the final layout. With debug on, we can easily see the effects of various adjustments, such as changing the width of the title and caption (`title_width`), altering the alignment of the caption (`caption_align`), or keeping the legend alignment of the original plot (`legend_shift`). Note that with `legend_shift = FALSE`, `finalize_plot()` does not separate the legend from the plot, as indicated by the debug boxes. 
...
### Exporting a graph with no title

One special case of the top-level position adjustments is the elimination of the title and caption altogether. This is generally preferred when creating graphics to embed in a Word/PDF report (as opposed to a web page or presentation slide). By setting `title_width = 0`, you can export a graph without the left title-caption column. If a caption is specified, it is moved below the plot element.

Note that in this situation, the constant `margin_title_l` is no longer relevant; `margin_plot_l` is now the margin between the left side of the image and the beginning of the plot. (See the next section for more details about these constants.)

```{r finalize6, message=FALSE}
# A finalized line graph, with no title or caption
finalize_plot(plot = p,
              caption = "Source: Chicago Metropolitan Agency for Planning
                analysis of Regional Transportation Authority data.",
              title_width = 0)
