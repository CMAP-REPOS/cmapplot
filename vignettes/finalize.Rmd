...
For this example, we will use the final version of the line chart developed in the article on `theme_cmap()` (see `vignette("plots")`). That plot is referenced in the following examples as `p`.
...
When naming your exports, you can include only the name of the file (e.g., `"my_chart"`) if desired - `cmapplot` will automatically add the appropriate extension to each of your exported files (e.g., `"my_chart.pdf"` and `"my_chart.png"`). If a file with that name already exists in the specified directory, they will not be overwritten unless the user sets `overwrite = TRUE`. Users should be aware that PDFs cannot be overwritten if the existing file is open.
