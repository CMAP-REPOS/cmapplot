#'Add Pandemics to time series graphs
#'
#'\code{geom_pandemics} returns one or two ggplot geoms that add rectangles
#'representing pandemics to a plot. It will either return only rectangles or,
#'by default, both rectangles and text identifying each Pandemic.
#'
...
#'  as a Pandemic.
#'@param label Char, the text to label each Pandemic. Defaults to " Pandemic".
...
filter_pandemics <- function(min, max, xformat, show_ongoing, pandemic_table) {
  ...
  if (xformat == "date") {
    ...
    if (xformat != "numeric") {
      warning(
        "geom_pandemics currently only supports x axes in the numeric and date formats. Using numeric."
      )
    }
...
GeomPandemics <- ggproto(
  ...
  setup_data = function(data, params) {
    ...
    data <- filter_pandemics(
      min = min(data$x),
      max = max(data$x),
      xformat = params$xformat,
      show_ongoing = params$show_ongoing,
      pandemic_table = params$pandemic_table
    )
...
GeomPandemicsText <- ggproto(
  ...
  required_aes = c("xformat", "label", "show_ongoing", "pandemic_table", "y"),
